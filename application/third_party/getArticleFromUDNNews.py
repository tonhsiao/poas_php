...
TD =  YD + datetime.timedelta(days=1) 

KeyWord_LL = ['遠傳','中華電','台灣大哥大','亞太電','台灣之星']
Total_LL = []
...
UDN_news_df = pd.DataFrame(Total_LL, columns =['發文者','標題','推回文類別','內容','發文時間','觀看次數','推噓評價','發文者分數','URL']) 
UDN_news_df['內容'] = UDN_news_df['內容'].str.replace('\n',' ').replace('\r',' ')


# In[ ]:


UDN_news_df.to_csv("/var/www/html/Fruit/pubCache/UDNnews_"+str(TD)+".csv", index = False)
#UDN_news_df.to_csv("UDNnews_"+str(TD)+".csv", index = False)
